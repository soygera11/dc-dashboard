...
df = load_csv("data/operations.csv")
last_row = df.iloc[-1]
...
st.plotly_chart(
    line_chart(df, "month", "uptime_sla", "Monthly Uptime SLA"),
...
with col_a:
    st.plotly_chart(
        bar_chart(df, "month", "open_incidents", "Open Incidents by Month"),
        use_container_width=True
    )

with col_b:
    st.plotly_chart(
        bar_chart(df, "month", "rack_utilization", "Rack Utilization Trend"),
        use_container_width=True
    )
